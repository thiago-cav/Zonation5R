...
This is a basic example to get a setup running with *Zonation5R*. Since this package serves as an interface, we need to make sure Zonation has access to the necessary input files. That means we must either use an on-disk directory or a writable temporary directory. To get started, let's copy the example data from the package into a working directory and make a quick plot.
